...
export interface HookConfig extends JWTVerifyOptions {
  ...
  stateHolder?: string;
  /** List of sources where to look for the JWT in the request. Could be the keyword `bearer` to check For Authorization Bearer token or `cookie:<cookie_name>` to check for cookies. Default is `['bearer']` */
  sources?: string[];
...
export const hook = (jwtConfig: JwtConfig) => {
  const { publicKey, sources } = { sources: ["bearer"], ...jwtConfig };
    ...
    let f = async (ctx: Context) => {
      ...
      for (let s of sources) {
        if (s === "bearer") {
          ...
          if (!auth) continue;
          jwt = auth?.replace(/^Bearer\s+/, "");
          break;
        }
        let m = s.match(/^cookie\:(.*)/);
        if (!m) continue;
        let cookieName = [...m][1];
        if (!cookies) {
          if (!ctx.headers?.["cookie"]) continue;
          cookies = cookie.parse(ctx.headers?.["cookie"]?.toString() ?? "");
        }
        if (!cookies?.[cookieName]) continue;
        jwt = cookies?.[cookieName];
      }
      let payload = null;
      try {
        ...
        return await conf.errorHandler(error as Error, ctx);
      }
    };
    hookInstances.add(f);
    return f;
...
export const plugin = (metadata?: { description?: string }) =>
  ({
    name: "jwt",
